...
# Remove unused import - exceptions module was cleaned up
...
class CompanyAnalystAgent(InvestmentAgent):
    ...
    prompt: PromptTemplate = PromptTemplate(
        ...
        **목표주가 산정**
        - **12개월 목표주가: $XX.XX** (상향/하향 여력 XX%)
        - **Bull Case: $XX.XX** (최적 시나리오)
        - **Bear Case: $XX.XX** (최악 시나리오)
        ...
        **시나리오 분석**
        - **기본 시나리오** (확률 60%): 목표주가 $XX.XX
        - **상승 시나리오** (확률 25%): 목표주가 $XX.XX
        - **하락 시나리오** (확률 15%): 목표주가 $XX.XX

        ## 🎯 **6. 투자 결론 및 실행전략**
        
        **최종 투자의견**
        - **레이팅: BUY/HOLD/SELL** (Strong Buy/Buy/Hold/Sell/Strong Sell)
        - **목표주가: $XX.XX** (현재가 대비 +XX% 상승여력)
        - **투자시계: 단기(3M)/중기(6-12M)/장기(2-3Y)**
        - **신뢰도: 높음/보통/낮음** (데이터 품질 기준)
